...
from local_orthoDB_tools import database

import local_env_variables.env_variables_and_filepaths as env


def ogid_2_geneid_list(ogid, db_path=env.DATABASE_FILES["OG2genes - sqlite"]):
...
def load_all_sequences_in_OG_ID(odbquery: database.orthoDB_query, ogid=None):
    ...
    id_list = ogid_2_geneid_list(ogid)
    og_seq_dict = {}
...
def add_species_2_seqid_from_full_SQL(odbquery: database.orthoDB_query):
    """
    Run on the full OG to avoid confusion
    """
    connection = sqlite3.connect(
        odbquery.odb_database.database_files["odb11v0 - sqlite"]
    )
    cursor = connection.cursor()
    seqrecord_dict = copy.deepcopy(odbquery.sequences_full_OG_dict)
    for seqrecord in seqrecord_dict.values():
        res = cursor.execute(
            f"SELECT species_id FROM genes WHERE gene_id='{seqrecord.id}'"
        )
        species_id = res.fetchall()[0][0]
        species_name = odbquery.odb_database.data_species_dict[species_id].replace(
            " ", "_"
        )
